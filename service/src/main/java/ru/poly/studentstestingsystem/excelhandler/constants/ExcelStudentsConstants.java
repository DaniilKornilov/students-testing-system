package ru.poly.studentstestingsystem.excelhandler.constants;

import java.util.List;

public class ExcelStudentsConstants {

    public static final List<String> STUDENTS_HEADERS = List.of("Идентификатор", "Имя", "Фамилия", "Почта", "Пароль");

    public static final int SHEET_INDEX = 0;

    public static final int HEADERS_ROW_INDEX = 0;

    public static final int USERNAME_COLUMN_INDEX = 0;

    public static final int FIRST_NAME_COLUMN_INDEX = 1;

    public static final int LAST_NAME_COLUMN_INDEX = 2;

    public static final int EMAIL_COLUMN_INDEX = 3;

    public static final int PASSWORD_COLUMN_INDEX = 4;
}
